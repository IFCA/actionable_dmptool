class PlansController < ApplicationController
  ...
  before_action :set_plan, only: [:show, :edit, :update, :destroy, :publish, :export, :details, :preview, :doi,:perform_review, :coowners]
  before_action :check_copy_plan_access, only: [:copy_existing_template]
  before_action :check_plan_access, only: [:edit, :update, :destroy, :details, :preview, :doi]
  ...
  def doi
    @plan.doi = create_doi(@plan)
    coowners
    create_metadata(@plan,@coowners)
    respond_to do |format|
        if @plan.save
            format.html { flash[:notice] = "DOI associated with #{@plan.name} created successfully: #{@plan.doi}."
  ...
  def details
    ...
    unless @requirements_template.nil?
      ...
      # Load taxonomy if is a taxonomy field
      @taxonomy = Taxonomy.find(@requirement.taxonomy_id) unless @requirement.taxonomy_id.blank?
