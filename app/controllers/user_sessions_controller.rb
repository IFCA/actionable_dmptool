class UserSessionsController < ApplicationController
  ...
  def login_fake
    ...
    user = User.where("login_id = '#{params[:username]}' and institution_id = '#{session['institution_id']}'")[0]

    if user.nil?
      redirect_to choose_institution_path, flash: {error: 'Username or Institution invalid. Please choose your log in institution.'} and return
    end

    password_hash = BCrypt::Engine.hash_secret(params[:password], user.password_salt)

    if user.password_hash != password_hash
      redirect_to choose_institution_path, flash: {error: 'Username or password not correct.'} and return
    end
