class UsersController < ApplicationController
  ...
  def create
    ...
    if existing_user && existing_user.length > 0
      redirect_to login_path, notice: "You already have a DMP Tool account with this email. Please log in with your current password to continue." and return
    end

    if !valid_password(user_params[:password],user_params[:password_confirmation])
      redirect_to new_user_path(@user), alert: 'Your password must be at least eight characters long and have at least one letter and at least one number.' and return
    end
    ...
    if @user.save
      ...
      redirect_to new_user_path(@user), alert: 'Unexpected error in user creation.' and return
    end
