  ...
  def response_text(plan) 
    unless plan.plan_responses_ids.blank?
      ...
      unless @response.blank?
        if (!@response.text_value.blank? and self.requirement_type.to_s == 'taxonomy')
          @value = ''
          @response.text_value.split(',').each { |id_concept|
            aux = (Viaconcept.find id_concept).complete_route_text
            @value = @value + aux
          }
          return @value
        elsif !@response.text_value.blank? and self.requirement_type.to_s != 'taxonomy'
    ...
    unless plan.plan_responses_ids.blank?
      ...
      unless @response == nil
        if (!@response.text_value.blank? and self.requirement_type.to_s == 'taxonomy')
            @value = ''
            @response.text_value.split(',').each { |id_concept|
              aux = (Viaconcept.find id_concept).complete_route
              @value = @value + aux + "<br/>"
          }
          return @value
        elsif !@response.text_value.blank? and self.requirement_type.to_s != 'taxonomy'
          @value = @response.text_value.to_s
        elsif !@response.numeric_value.blank?
          @value = @response.numeric_value.to_s 
        elsif !@response.date_value.blank?
          @value = @response.date_value.to_date.strftime("%m/%d/%Y")
